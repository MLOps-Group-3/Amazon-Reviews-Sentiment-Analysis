...
class MetricsCallback(TrainerCallback):
    def __init__(self, metrics_logger):
        ...
        Args:
            metrics_logger (callable): A function or callable to log/store metrics.
        """
...
def initialize_bert_model(num_labels, model_name="bert-base-uncased"):
...
def train_bert_model(model, train_dataset, val_dataset, output_dir,metrics_logger=None, **training_args):
    default_args = {
        "learning_rate": 2e-5,
    ...
    # Add a callback for logging metrics
    if metrics_logger is not None:
        trainer.add_callback(MetricsCallback(metrics_logger))
...
if __name__ == "__main__":
    ...
    # Initialize BERT model and move it to the appropriate device (CPU or GPU)
    model = initialize_bert_model(num_labels=len(label_encoder.classes_))
    ...
    # Train the model
    eval_results, trainer = train_bert_model(model, train_dataset, val_dataset, OUTPUT_DIR, **training_args)
