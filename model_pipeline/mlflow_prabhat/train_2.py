...
# Set up MLflow
mlflow.set_experiment("Sentiment Analysis with BERT")
...
def create_data_subset(df, subset_size=0.1):
    logger.info(f"Creating a subset of {subset_size*100}% of the data")
    return df.sample(frac=subset_size, random_state=42)
...
class SentimentDataset(Dataset):
    def __init__(self, texts, titles, prices, price_missing_col, helpful_votes, verified_purchases, labels, tokenizer,
                 max_length=128):
        self.texts = texts
        self.titles = titles
        self.prices = prices
    ...
    def __getitem__(self, idx):
        ...
        additional_features = torch.tensor(
            [self.prices[idx], self.price_missing[idx], self.helpful_votes[idx], self.verified_purchases[idx]],
...
def initialize_model(num_labels):
    ...
    model = CustomBERTModel(base_model, num_labels=num_labels)
...
def train_model(model, train_dataset, val_dataset, output_dir='./saved_model'):
    ...
    with mlflow.start_run():
        ...
        mlflow.log_metrics({f"train_{k}": v for k, v in train_results.items()})

        eval_results = trainer.evaluate()
        logger.info(f"Validation set evaluation results: {eval_results}")
        mlflow.log_metrics({f"val_{k}": v for k, v in eval_results.items()})
...
def evaluate_on_test_set(trainer, test_dataset, label_encoder):
    ...
    with mlflow.start_run(nested=True):
        ...
        for i, (p, r, f, s) in enumerate(zip(precision, recall, f1, support)):
            ...
            mlflow.log_metrics({
                f"test_precision_{label_name}": p,
                f"test_recall_{label_name}": r,
                f"test_f1_{label_name}": f,
                f"test_support_{label_name}": s
            })
...
if __name__ == "__main__":
    with mlflow.start_run():
        ...
        mlflow.set_tag("dataset", "Amazon Reviews")

        # Define paths and settings
        data_path = "labelled_data.csv"
        ...
        df_subset = create_data_subset(df, subset_size)

        # Split data
        train_df, val_df, test_df = split_data_by_timestamp(df_subset)

        # Initialize tokenizer
        tokenizer = initialize_tokenizer()

        # Prepare datasets
        train_dataset = SentimentDataset(train_df['text'].tolist(), train_df['title'].tolist(),
