...
#         positive_count = sum(row["relevant_sentences"].lower().count(word) for word in positive_words)
#         negative_count = sum(row["relevant_sentences"].lower().count(word) for word in negative_words)
...
#     labeled_data.to_csv('milestones/data-pipeline-requirements/03-data-preprocessing/staging/data/labeled_data/labeled_data_aspect.csv')
